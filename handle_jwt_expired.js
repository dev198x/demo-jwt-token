...
axios.interceptors.request.use((req) => {
  ...
  console.log('axios.interceptors.request.use requestStartedAt>>...')
  return req;
});

axios.interceptors.response.use((res) => {
  res.durationInMs = new Date().getTime() - res.config.meta.requestStartedAt;
  console.log('axios.interceptors.response.use durationInMs>>...')
  return res;
});
...
(async () => {
  try {
    ...
    console.log(
      `Github username ${githubUserName} has real name: ${data?.name} and works at ${data?.company}`
    );
